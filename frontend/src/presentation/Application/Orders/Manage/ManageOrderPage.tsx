...
const ORDER_ITEM_STATUS_COLORS = {
    [OrderItemStatus.FINISHED.value]: "bg-green-400",
    [OrderItemStatus.PENDING.value]: "bg-orange-400/70",
};

const ORDER_STATUS_COLORS = {
    [OrderStatus.FINISHED.value]: "bg-green-400",
...
function OrderItem(props: { orderItem: IOrderItem }) {
    ...
    return (
        <MixinPrototypeCardSection className="flex flex-col gap-2" key={orderItem.id}>
            <div className="w-full flex flex-row justify-between px-2 py-1 bg-gray-200 shrink-0 rounded">
                {Array.from({ length: 4 }).map((_, i) => (
                    <CoverImage
                        className="h-12 w-12 border border-gray-900 rounded overflow-hidden bg-gray-50"
                        src={
            ...
            <div className="flex flex-col gap-1 overflow-hidden max-w-full">
                <div className="flex flex-row gap-2 items-center text-base">
                    <div className="text-sm font-bold bg-gray-200 px-2 py-1/2 rounded">x{orderItem.quantity}</div>
                ...
                <div className="flex flex-row gap-2 items-center text-base">
                    <div className={`text-sm ${ORDER_ITEM_STATUS_COLORS[orderItem.status.value]} px-2 py-px rounded`}>{orderItem.status.value}</div>
